...
def mv_gaussian(wgt_gss, mu_gss, chol_gss):
    ...
    for n in range(n_steps):
        for m in range(n, n_steps):
            L[m*n_gss:(m+1)*n_gss, n*n_gss:(n+1)*n_gss] = np.matmul(An_m[:, :, m, n+1], chol_gss[:, :, n])
        for m in range(n+1):
            gaussian_mu[n*n_gss:(n+1)*n_gss] = gaussian_mu[n*n_gss:(n+1)*n_gss] + An_m[:, :, n, m+1].dot(mu_gss[:, m])
